...
namespace Dialog {
    ...
    void dialog() {
        ...
        std::cout << "At the moment, " << signal.get_SZ() << " signal states are defined." << std::endl;
    ...
    void Add_signal(Binary_signal &signal){
        ...
        try{
            Signal_status sig{symbol,time};
            signal += sig;
            std::cout << "\nSignal status with " << symbol << " added to location!" << std::endl;
