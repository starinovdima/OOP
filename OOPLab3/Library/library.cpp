...
namespace Laba3{
    ...
    void Binary_signal::add_signal_to_end(unsigned char symbol, double time){

        if(time + time_check() > max_duration)
        ...
        status[place_check()].value = symbol;
        status[place_check()].seconds = time;

        std::cout << "\nSignal status with " << symbol << "added to location " << place_check() << std::endl;

    }



    void Binary_signal::cleaning_all(){
    ...
    void Binary_signal::add_signal(const Binary_signal &signal_two) {
    ...
    Binary_signal &operator ~ (Binary_signal &signal) {
        for (int i = 0; i < Binary_signal::get_SZ(); i++){
            if(signal.status[i].seconds != 0){
